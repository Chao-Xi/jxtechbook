...
1. **inode**
    软链接：**不同inode** / 硬链接：**相同inode**
2. **跨分区**
    软链接：**支持**跨分区、跨磁盘 / 硬链接：**不支持**，只能同分区
3. **目录链接**
    软链接：**可以**链接文件夹 / 硬链接：**不能**链接目录
4. **删原文件**
    软链接：原文件删完**失效**（红报错） / 硬链接：原文件删除**依然可用**
5. **大小**
    软链接：大小=**路径字符长度** / 硬链接：大小**和原文件一致**
...
### 117 Load Balancing Algorithms


![Alt Image Text](../../images/2026sre_1_117.gif "Body image")
...
```
Client Requests → LB → Server 1 → Server 2 → Server 3 → Server 1 → ...
```
...
```
Server 1: 10 conns
Server 2:  3 conns  ← LB picks this
Server 3:  7 conns
```

* ✅ Good for long-lived connections (DB, WebSocket)
...
Round Robin, but servers get traffic **proportional to their weight**.

```
Server 1: weight 5  → gets 50% of traffic
Server 2: weight 3  → gets 30%
Server 3: weight 2  → gets 20%
```

* ✅ Useful for heterogeneous hardware
* ❌ Weights are static (manual tuning)



**4. Weighted Least Connections**

Combines **least connections + weight** — picks server with lowest `(active_conns / weight)`.

```
Server 1: 10 conns / weight 5 = 2.0
Server 2:  4 conns / weight 3 = 1.33  ← LB picks this
Server 3:  2 conns / weight 1 = 2.0
```

* ✅ Best for mixed-capacity environments
* ❌ More complex to configure


**5. IP Hash**

Hash the **client IP** → always routes same client to same server.

```
hash(203.0.113.5) % 3 = 1  → Server 1
hash(198.51.100.7) % 3 = 2 → Server 2
```

* ✅ Session persistence (sticky sessions) without cookies
* ❌ Uneven distribution if few clients; breaks if server pool changes


**6. Least Response Time**

Routes to the server with the **fastest response time** (often + fewest connections).

```
Server 1: 10 ms
Server 2:  5 ms
Server 3:  1 ms  ← LB picks this
```

* ✅ Best for latency-sensitive apps
* ❌ Requires health/latency monitoring; can cause flapping
...
* ❌ Can create short-term imbalance


**8. Least Bandwidth**

Routes to the server currently using the **least bandwidth** (Mbps).

```
Server 1: 200 Mbps
Server 2:  50 Mbps  ← LB picks this
Server 3: 100 Mbps
```

* ✅ Good for media/streaming workloads
* ❌ Requires bandwidth monitoring; slower to react


**Quick Comparison Table**

| Algorithm | Best For | Sticky? | Considers Load? |
|-----------|----------|---------|-----------------|
| Round Robin | Uniform servers | No | No |
| Least Connections | Long-lived conns | No | Yes |
| Weighted RR | Mixed capacity | No | Partial |
| Weighted Least Conn | Mixed capacity + load | No | Yes |
| IP Hash | Session persistence | Yes | No |
| Least Response Time | Low-latency apps | No | Yes |
| Random | Large pools | No | No |
| Least Bandwidth | Media/streaming | No | Yes |

One-Liner Summary

> Load balancers pick a backend using algorithms ranging from simple **Round Robin** to smarter ones like **Least Connections** and **Least Response Time**, with **IP Hash** providing session stickiness.

### 118 Infrastructure as Code with Terraform


![Alt Image Text](../../images/2026sre_1_118.jpeg "Body image")

**1. What is Terraform?**

- Open-source IaC (Infrastructure as Code) tool by **HashiCorp**.
- Provisions and manages infrastructure across multiple cloud providers using **declarative configuration**.
- Uses **HCL** (HashiCorp Configuration Language).

**Key Benefits:** Automation, Consistency, Scalability, Version Control.

**2. Core Concepts**

| Concept | Purpose |
|---------|---------|
| **Providers** | Interact with cloud/infra (AWS, Azure, GCP, K8s) |
| **Resources** | Create/manage infra (VMs, VPCs, S3, etc.) |
| **Data Sources** | Fetch existing data (AMI ID, VPC ID) |
| **Modules** | Reusable configuration blocks |
| **State** | Tracks managed resources |
| **Variables** | Make config dynamic |
...
- `terraform.tfvars` → variable values (optional, don't commit secrets)
...
- **Remote backend** recommended for teams (S3, Terraform Cloud, etc.).
- Use **state locking** (e.g., DynamoDB) to prevent concurrent runs.

**Best Practice:** Never edit state manually. Always use a remote backend for teams.
...
- Reusable, self-contained config packages.
- Can be local (`./modules/vpc`) or from a registry.
- Example:
...
**Benefits:** Reusability, Maintainability, DRY.
...
| Azure Storage | Built-in |
| GCS | Built-in |

**Benefits:** State locking, collaboration, security, versioning.
...
- ✅ Manage secrets securely (Vault, AWS Secrets Manager, SOPS)
- ✅ Version control everything (Git)
- ✅ Run `terraform fmt`, `validate`, and `plan` before `apply`
- ✅ Pin provider versions
- ✅ Use `-target` sparingly

**Pro Tip:** Treat your Terraform code like application code!
...
terraform state show   # Show a resource
...
| **Overridable?** | Yes, easily (at runtime) | Yes, but only with `--entrypoint` flag |
| **Dockerfile Syntax** | `CMD ["echo", "Hello"]` | `ENTRYPOINT ["echo"]` |
| **Runtime Behavior** | Everything after the image name replaces CMD | Arguments after the image name are appended to ENTRYPOINT |
| **Best Practice** | Use for default parameters | Use for the application binary |


**How They Work Together (The Golden Rule)**

When both are used in a Dockerfile, **ENTRYPOINT defines the command, and CMD provides the default arguments.**
...
| :--- | :--- | :--- |
| `docker run myimage` | `Hello Docker` | Uses ENTRYPOINT (`echo`) + default CMD (`Hello Docker`) |
| `docker run myimage Welcome` | `Welcome` | `Welcome` replaces the CMD arguments (`Hello Docker`) |
| `docker run --entrypoint ls myimage` | `bin dev home...` | `--entrypoint ls` overrides the ENTRYPOINT, leaving CMD empty |


**Key Takeaways (From the Image)**

- **CMD** = Default arguments (easy to replace).
- **ENTRYPOINT** = **Main command (harder to replace, ensures the app always runs).**
- **Best Practice:** Use `ENTRYPOINT` for the application binary and `CMD` for default parameters.


**Important Technical Nuances (Interview Bonus Points)**

1.  **Shell vs. Exec Form:**
    - `CMD ["echo", "Hello"]` (Exec form) → Runs directly, no shell. PID 1 is `echo`.
    - `CMD echo Hello` (Shell form) → Runs in `/bin/sh -c`. PID 1 is `sh`, and `echo` is a child process. This can cause signal handling issues (e.g., `SIGTERM` not reaching your app).
    - **Always prefer the Exec form `["..."]`** for production apps.

2.  **Multiple CMDs/ENTRYPOINTs:**
    - A Dockerfile can only have **one** `CMD` and **one** `ENTRYPOINT`. If you specify multiple, only the last one takes effect.

3.  **Overriding ENTRYPOINT:**
    - You *can* override `ENTRYPOINT` at runtime using `docker run --entrypoint <new-command> <image>`, but this is generally discouraged unless you are debugging.

4.  **Kubernetes Context:**
    - In Kubernetes, the `command` field overrides Docker's `ENTRYPOINT`.
    - The `args` field overrides Docker's `CMD`.

**Easy Way to Remember**

- **CMD** = **C**an **M**odify **D**efaults (easy to replace)
- **ENTRYPOINT** = **E**nsures **E**xecution (harder to replace)

**Pro Tip:** Treat `ENTRYPOINT` as the "verb" (what to run) and `CMD` as the "adverb" (how to run it by default).
