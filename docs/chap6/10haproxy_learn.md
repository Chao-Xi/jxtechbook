...
* **使用3.x内核的Linux操作系统运行HAProxy运行HAProxy的主机上不要部署其他的应用，确保HAProxy独占资源**，
...
如果发现仍然被定位到ms1.srv1，同时也没有写入新的`HA_STICKY_ms1 Cookie`，那么可能是浏览器缓存了`ms1/demo.html`页面，请求并没有到达HAProxy。F5刷新一下应该就可以了。
