...
我们的业务数据，**先不往关系型数据库里面去写，而是先把数据写到redis里面**，然后再根据需要，按照场景来做一些这个持久化的同步

常见的redis的场景,主要是读多写少, 利用缓存的懒加载机制就可以了, 先去redis里面查, 而如果没有,就去库里面去捞,然后把数据刷到缓存里面

redis只是作为一个辅助的手段,来提高查询的效率. 

* 点赞
* 实时的调API


去修改数据库,往库里面去增加记录,包括修改它的一个总量, 就是在redis里面去维护一个key, 来存储当前文章,  性能就会大大的提升, 它的性能要高好几个数量级

然后再设置好定时任务, 做好redis跟数据库之间的个同步.  同样也可以采用redis来作为主库, 就是会有频繁的用户交互. 都属于读多写多的场景

**常见的这个数据的一致性问题 , redis跟Mysql, 同步是有时间差,怎么去保证一致性** 

**以Redis做主库, 比较适合一些交互性比较高，又对一致性要求没那么高的场景，比如说常见的文章浏览量**
...
那么一旦主结点挂了，那么哨兵sentinel对就会把主节点下掉， 虽然主从模式也有多个服务也是一个集群。实际上真正对外服务，只有一台机器，**所以如果公司采用了redis的这个主从模式**，或者说是哨兵模式，redisn符合CP

* **Redis 3.0以后**

redis cluster 模式，可以实现数据分片， 每个节点呢存储一部分数据，每个数据分片又有多个副本。 然后分散在不同的节点中，可以同时对外提供服务， 可以实现故障的转移，还有其他节点可以继续提供服务，但是引入了多副本到了数据的同步。

当某个数据发生了变化，就会把这个修改的这个节点上的数据， 同步到其他的节点，但是由于网络传输，延迟啊等等原因。 不一定会被其他节点接收到， 所以可能会导致节点之间存在数据不一致的情況，当然了如果某一个节点发生了故障

瞬时产生的这个数据，是无法同步到其他从节点的，所以当这个节点恢复之后，重新加入到集群，那么之前没有同步的数据。只会存在之前故障的那个节点中，其他节点就没有这部分数据了，也会产生不一致。**所以我们说cluster模式其实是AP**

**它是能够保证可用性和容错性的，它的一致性是保证不了的**，因为它涉及到一个数据同步的问题，只能算是最终的一致性

### 21 如何保证redis中的数掘 都是热点数据

优先保证所有的热点数据也就是用户访问此较多的数据, 放在缓存里面. 

我们在设置一个key的时候, 都会给这个key设置一个过期时间, 比如说key的过期时间为一个小时,那么一个小时到了. 这个key就过期子访问不到了.

如果在到期之前被访问到了, 那么就会增加一定的这个过期时间. 增加30秒或者一分钟,访问越多过去的时间呢就会越长, 留在这个redis里面的. **自然就是热点数据了**. 

这些过期的key,也是额外有一个删除的策略，叫做**定期删除**。默认是每隔啊100毫秒，随机抽取一些设置了过期时间的key，来检查是否过期。如果过期就直接删除，**但是它不是每隔100毫秒就遍历所有设置过期时间的key**，redis它是每隔100毫秒，随机抽取一些这个key的检查来删除的

当然定期删除可能会导致很多的过期的key, 到了时间, 并没有从redis里面给清掉, 就需要做这个**惰性删除**, 就是在获取某个key的时候, redis它会检查, **这个key如果过期**, 那么就会直接从的这个redis里面给它删除掉.

**定期删除、惰性删除两者结合起来使用, 就能够保证key过期, 相应的内容也能够从redis里面删除掉, 留存下来的就是我们的热点数据.**

但是如果定期删除也漏掉，很多过期的key，然后也没有及时的去做查询，也没有走惰性删除，**可能就会有大量的过期的key堆积在内存里面导致内存的耗尽**，就可以走这个内存淘汰机制（10 redis缓存过期策略）。用的比较多的就是lru，就是最近最少使用，就是最近未使用的或者使用比较少的key，会直接通过淘汰机制来删除掉，保江有充足的空间留给热点数据

**当然除了过期的时间策略以外，还有一个肉眼可见的热点数据就会直接刷到内存里面，然后定期更新，不需要再从业务模块里面啊
**
...
分布式框架引入很方便，但是同样也带来了很多的问题。比如说系统会变得越来越复杂。 系统的维护成本也很高，版本迭代发布的也变得越来越复杂。还有最常见的就是cap问题, 一致性的问题等等, 它不是立即被删除的
...
RPO RTO是在系统高可用灾备设计的时候。 经常会考虑的两个重要的指标

* RPO是恢复点数据的指标，就是这个系统如果出现了故障，那么最多可能丢失多长时间的数据
* RTO 另外一个是恢复运行，这个时间的指标，就是从系统挂了不可用，到恢复正常运行的过程。比如说每隔6个小时来备份一次。 RPO 的最长时间6个小时

**可用性角度来说RPO RTO，肯定是越低越好了，系统的可用性肯定是越强了**

### 27 跨中心查询帶来的性能问题该如何解決？

跨中心组装数据，比如API.  需要同时调用两个以上的业务中心, 才能够拿到你想要的数据. 

**性能问题**经常就会出现在, 那怎么解決跨中心调用的性能问题.

* 就是利用批处理, **就是将你的订单数据我们的内存中，完成这个数据的组装**，返回给客户端，这是一种解決方式
* **那么如果遇到数据量大， 通过批处理还是接口响应慢怎么办**
	* 就可以通过缓存的方式， 在首次读取的时候。 将计算好的结果集缓存在redis里面， 在下次读取的时候，就可以直接读取缓存中的数据了
	* redis跟数据库的这个双写一致性。 那么在用户下单的时候， 需要额外做一个步骤，就是同步好redis里的数据
	*  **也可以在库里面来建这个汇总表，将结果集放在汇总表里面**，就可以通过消息队列，或者其他什么异步的方式来同步，**更新汇总表就可以了**，那么这样在查询的时候，就可以直接查询汇总表，而不是再去查各个中心，都是属于这个典型的空间换时间
...
应该算是消息队列的灵魂，一个负责消费消息，一个负责产生消息， 

一个接口，来供生产者发布消息，要满足这个先进先出，可以使用链表，数组或者是数据库来实现

数据格式，一般采用这个json， 对于这个序列化，反序列化。

 那消息产生出来之后存在哪里，redis, **天生就有这个持续化的机制.** 
 
 然后就是这个消息的消费. 就得有一个AP!来提供. 消费者从队列中获取、消费消息，可以使用Pull（拉）， 或者是push（推）模式
...
队列需要在要产生了新消息之后，主动给推送到我们，


**怎么保证消息不会重复的消费**， 这里可能就要使用到锁，这里可能就要使用到锁或者是信号量来控制，为了确保消息被消费者接收处理。还需要有一个这个确认机制，核心点就在子我们的消费者

在处理完消息后，要发送确认，就是常说的这个ACK，那么队列在接收到确认消息，那么队列在接收到确认消息，消费之后呢，才可以将我们队列中的消息进行删除， 

如果出现异常， 消费者在处理消息时崩溃，那么消息就不应该被删除，而是应该在消费者恢复后，再做消息的补偿发送

**当然还有队列顺序消费的问题**，在某些场景下呢，消息的顺序是很重要的

**就是这个消息过期**，对于长时间末消费的消息，要把它过期掉，可以设置这个过期时间，过期后，移至这个死信对列，设计一个死信队列的消费者，来专门处理这些异常消息的消费
...
线上慢SQL的原因，比如说常见是**没有加索引或者是索引未命中**。 索引没命中，数据库就要执行全表扫描。

所以有意识地去加索引，即使加了索引也要去检查索引到底有没有失效，素引失效的场景也有很多。比如说索引列用了**计算函数， 联合索引不符合最左匹配， 不怡当的模糊搜索， 范围检索等等**

还有一个慢sql的场景，就是多表关联。即使说都命中了索引，扫描的行数仍然是巨大的（数据量大时）执行速度仍然会受到很大的影响。**一般超过3个表，尽量的避免需要将它拆成多个查询，也就是常说的这个空间换时间**

可能产生慢sql的因素，**就是遇到了行锁或者表锁**。

* 那表锁就是锁住了整个表，一旦表锁住，那么其他用户需要等待锁释放，才能进行操作，这样的话就会导致性能极大的一个下降。如果锁定的时间过长，可能还会导致整个系统的崩溃，也会有一些这个监听的程序，来防止出现锁表的情况啊
* 那么对于行锁， 一般都是事务发生阻塞。如果一个事务锁住了某条记录，那么另一个事务，就无法对它进行修改，只能阻塞。如果无法快速的释放，那么很容易就变成了一个慢sql

当然如果命中了，索引也没有出现多表的关联，也没有遇到行锁表锁，查询还是慢，也没有遇到行锁表锁，查询还是慢，也没有遇到行锁表锁，查询还是慢，**是不是各个中间件有网络延迟**，**是不是查询内容过多**，**是不是每次都要回表**，同样也会引起慢SQL的产生

### 16 SQL优化 不同的字符集引起的索引失效

**命不中索引，走的全是全表扫描** 那么最后定位下来， 竟然是两个表的字符集不一致。

一个用的是UTF8，一个用的是UTF8MB4，**但是两者是属于完全不一样的宇符集**，UTF8MB4的特殊字符集UTF8就不支持。  **所以在关联查询的时候， 就是将这个字符由UTF8转换成UTF8MB4**

![Alt Image Text](../images/chap7_arch_2.png "Body image")

这个就是索引失效的这个原因所在，**因为使用了这个convert函数**

一般去分析索引失没失效，都是直接通过在执行语向前面，用explain,来看索引有没有命中,看不了太多的内容啊

`explain format=json`,再加上你的SQL

### 17 数据库中使用临时表会带来那些问题

当初使用临时表的目的，就是为了能够像真实的数据表里一样，可以用聚合，关联，排序，可以省写一堆业务代码的时间。 

使用临时表目的，就是为了省事。只要量不大，临时表是可以在内存做一些处理的，索引的使用也是有效的。但是一旦量大了，他是无法将数据全部存储在内存里的。 

Mysql就会将部分的数据写入到磁盘, 来释放这个内存的空间. 相比于内存, 磁盘的访问的速度慢下来了. 索引效率也会受到限制,查询大概率呢导致全表扫描,速度也就慢了. 其实想要针对这个内存中的数据,做一些聚合, 关联计算可以使用.  像这个java里面的这个lambda表达式, Java 8以后的版本. 也都支持这样的处理方式. 没必要非要用临时表才能处理.

如果数据量比较大, **那么你创建查询啊临时表,**  还会导致频繁的这个磁盘IO, 会降低查询的性能, 那么如果API又存在这个并发操作, 那么频繁的创建销毁.  可能会引发更大的这个性能问题, 临时表无法使用分布式事务。 

发现Seata是不支持临时表，临时表创建完，会立刻插入数据，Seata又无法立刻获取到表的结构，它的元数据是无法获取的，那么也就无法生成反向的sql，那么在后期自然也就无法回滚数据

**管理器查不到临时表的元数据会直接报表不存在，所以在分布式事务的场景，临时表一般也是禁止使用的**

### 18 整据库中的视图的利弊

* **Pros**

视图的主旨是为了使用一个虚拟表，来代替复杂的查询sql，**可以把它当成一个数据表来查**，它是照样执行了一堆sql，**就是把查询的过程，结果它是做了个包装**，用关心内部怎么查的，**把它当成一个真实的数据表**，**简化了查询的复杂度**

直接通过视图，就可以进行数据访问了, 能够保证数据的一个完整性, 当然也可以在一定程度上, **能够保证数据的一个完整性, 一致性**, 因为只要底层的元数据更新了整个视图， 也都是实时能够查到更新后的数据，不用像存储在统计表或者是缓存里面还需要去关注数据的一致性。 

还可以用来限制用户的访问权限，可以直接开放试图的查询给用户，而不用把元数据的权限给开放出去, 能够保证数据的稳定

* **Cons**

* 仅仅利用它去查询某一个场景的数据,  **但是直接利用数据库中的脚本来查**。
* 就是性能方面的开销， 对性能是有方面的影响。 而视图仅仅只是查询的过程，它是没有办法使用索引的，所以一旦量比较大，或者说是上升到一定的这个数据量查询会越来越慢

### 19 ACID的理解

**ACID原子性,一致性,隔离性, 持久性**

* 原子性: 数据库的事务最小工作单元，要么全部执行，要么全部不执行，**早期没有分布式事务的情况下**， 一般都是利用数据库的这个事务，来保证操作的原子性，手动开启一个事务。那么单体架构中采用像本地事务这个transactional，**他的注解其实映射在数据库层**，也就是统一的这个事务处理
* **一致性**：是一种规则的约束， **就是整个数据库的这个操作**，事务的操作是符合预期的，所有的数据的状态必须是有效的而且是正确的，它更多的是一种规范的约束
* **隔离性**： **井发层面，那么一般默认都会采用读已提交**，就是在别的事务末提交之前，**你访问到的所有的其他线程的这个数据都是老数据**，主要就是为了解决数据库一个脏读的问题
* **持久性**： 就是强调数据不丢失啊，即使发生了硬件的故障或者是数据库的这个崩溃，那么写入的任何数据也不会丢失，只要你的事务提交了数据就肯定不会丟


### 20 数据库中的触发器

用例： 如果库存没了，那么就直接通过前置触发器拋出异常，所以return掉

**那么这种做法**，多了一道检查，如果库存为0，就发现触发器，暴露出来的问题越来越多。

**很多时候喜欢把业务写到触发器里面**，在代码控制逻辑控制上，越来越困难，比方说统计商品浏览量，就喜欢通过触发器的方式，在添加浏览记录之后。

让触发器直接来做加一的动作，但是对于统计次数的这种，觉得就没有必要去占用，数据库的实时处理的资源，可以通过这个异步的方式，消息队列的方式，来处理就可以触发器的实时处理，影响了整体数据库的性能

在维护层面，跟存储过程一样，脚本写在数据库。它其实是很难理解和维护的，而且对于错误的排查上也比较困难。如果处理不当，还容易产生数据不一致的问题等等，触发器该舍弃就舍弃。

### 21 数据库中 脏读，幻读，不可重复复读

#### 脏读

一个事务读取了另外一个事务末提交的数据, 比如说当一个事务修改了某行数据, 但是还未提交, 那么另外一个事务又读取了这个末提交的数据, 就跟实际在库里面的值有了出入. 

已经解决了，默认的务隔离级别 =》 **就是这个读已提交**

就是说一个事务，他只能读到已经提交的数据。

#### 怎么解決幻读

幻读来讲通常发生在一个事务在读取一个范围内的数据。那么另外一个事务，在这个范围内插入了新的数据或者删除了原来的数据。就导致前一个事务呢，在后续的读取中看到了，在事务开始时从来没有的数据，或者说缺失了原有的数据。 相当于前一个事务产生一种错觉，数据被其他事务修改了，导致了前后读取不一致的问题

> 不实际的解决方案： **可以采用这个串行化的隔离模式**，所有的事务全是排队执行，完全是串行化的处理，就变成了多个事务在那排队
> 
> 效率，性能各个方面，都会受到很大的影响

采用这个乐观锁，**为数据来增加版本号或者时间戳**，那么在事务结束之前，去检查读取的记录，是否发生了变化。如果发生了变化，就可以选择回滚事务，或者是重新读取数据，来避免幻读和不可重复读的问题


当然也可以采用这个分布式锁，锁住同样需要处理的资源

### 22 聊聊整据库中的MVCC

在数据库领域还有一种无锁的方案， 可以来解决并发控制的问题，那就是传说中的MVCC。 不是业务层面的这个并发控制，还有数据库一致性方面的问题, **其实从它的定义来看就是多版本的并发控制**，它强调的是多版本，Multiversion，其实这个多版本，针对的是一个数据行， 只要事务开始了这个数据行，基本上都会生成一个版本，又叫做快照

那么在同一个事务下面，新读取的所有的数据，它都会产生一个快照，那么对于可重复读的这个隔离场景

第一次读取，生成快照以后，后面再读。每次查询都会返回之前生成的那个快照， 就不会产生不可重复读的现象了，因为前后都是一致的

**MVCC的****核心就是这个快照，那这个快照存在数据库哪儿**，存在underdog 表里面（回退日志表），是数据库事务中必不可少的表

在整个数据库事物的场景中， 所有关于数据库的操作记录，都会写在的undolog表里面，事务在执行过程中发生了异常。 那就会根据的这个undolog表，去发生回滚的

那么我就会先在undolog表里面去查找看看有没有这个快照可以直接返回，如果有那么就直接返回，如果没有，再去查找原生数据库，undolog表里面，它都会记录数据的主键。

事务的ID，还有上一个快照的地址，就会形成一个快照链表，再根据事务的隔离级别，根据这个事务提交的状态，自然就能很快的找到你想要的快照


### 23 binlogo, redo logo. undolog 的理解以及实际应用场昜

**binlog**

它是用来记录数据库所有的变更操作，像什么数据的**新增，修改，删除**，都会实时写入这个binlog里面。  像这个读写分离，主从复制， 都会涉及到它

那么对于主从数据库来讲，**一般是主库要负责写，从库要负责读**，那么主库怎么向从库同步数据，就会用到这个binlog。 让从库来监听主库的这个binlog，当binlog有了变动，**那么就会直接在从库中执行一遍这个更新操作**，就能够完成主库，到从库这个数据同步

**undolog**

回滚日志，主要是用于对事务的回滚，开启一个事务之后，所有针对数据库的操作，都会记录到这个undolog里面，一旦事务过程中发生了异常。那么数据库，就会根据当前事务的这个undolog，来回滚数据库的操作

**redolog**

重做日志，其实在事务层面

它是在数据库发生故障以后，用于数据的这个恢复来用的。同样保存的，也是针对数据库的这个更改操作，只是这个更改，**针对的是物理层面，记录的是磁盘数据页上的变化**，那么在MySql崩溃后， **要去恢复它**，就是这个redolog和binlog，来对这个mysql进行恢复的,保证了数据的完整性和一致性


### 24 哪些情況下不要使用卖引

**什么会有这种考虑**，其实还是要取决于，**使用索引会带来多少提效，会耗费多少成本**，都是来自于维护数据时

比如说每次的插入，更新或者删除，那么除了对数据表的操作以外，**数据库还需要更新索引数据结构**，就会导致对磁密的一些额外写入操作，此如说增加了这个IO成本，当然还有索引的这个存储成本了

我们评判要不要使用索引的标准，建立索引能不能带来很大的性能提升

如果效果不大或者说带来了负提升，那么我建立这个索引是没有意义的，不太适合使用索引的场景

#### 1、小表

可能也就几十条，上百条的记录，如果还进了很多索引，其实是属于负提效。反而在维护索引的时候，会增磁盘的IO，影响了数据库的查询性能。

#### 2、 频繁更新的表

或者存在大规模的批量操作的表，比如说大量的数据的插入，删除，更新也是不适合建立索引的。那么每次更新操作，可能会导致索引，重建或者更新，因为频繁的更新.  b+树也会频繁的更新, 带来的这个维护成本同样也会很大, 就是对于大量重复的数据的这个字段


比如说常见,的就是这个用户表里面的性别.索引

在查询的时候会定位很多数据, 查询上会有一些提升, 可能会超过索引带来的性能的改进, 还有无序的字段. 比如说像常见的这个哈西, 或者是uuid, 那么这类新插入的数据,会导致索引内部的位置变动，**因为它是无序的**，就会触发索引页的这个分裂，一般不会短，像uuid呢有32位，索引的额外存储成本加大，还是需要谨慎考虑啊

#### 3、 不等于或者是函数

是否需要建立索引，还有一些查询条件上的字段，此如说用到了这个**不等于或者是函数**

像不等于这种条件， 是无法使用排序好的索引，可能会选择进行全表扫描，来获取符合条件的记录，而不是使用索引，函数。用到计算肯定会走全表扫描

### 25 教据库中的**主键跟索引**有什么关联

其实对于主键来讲, 它的主要目的是单个宇段或者是多个字段的组合, 它要保证表中的每一条记录, 都有一个独有的主键值, 而且不允许为空

那么对于索引来讲,  加快对表中数据的这个访问速度, 加快对表中数据的这个访问速度。

索引可以是一个字段，或者是多个字段的组合，并且通过维护一种数据结构， **常见的像这个b+树**或者哈希表，加速对表的这个记录的检索，数据就可以使用索引。 

当应用查询的时候，数据就可以使用索引，此较快捷的定位到符合条件的记录，就不需要扫描整个表了，在大多数啊关系型数据库中，只要主键建立成功了，都会自动的创建一个独有的索引，来确保主键的快速访问，我们称这些索引做主键索引

当然其他的一些非主键的字段，也可以通过创建索引，来提高对这些宇段的这个查询效率。其中也包括了这个unique索引， 它有点类似就是主键在建立的同时，也就是主键索引。

全键索引跟非主键索引，他们在查询的时候，也会有些关联，在非全键索引命中查询的时候，**数据库的引擎首先就通过非主键索引来定位**，记录的位置，然后通过该位置的主键值，再次访问主表，主键索引来获取进入到其他的这个列的数据

**这个额外的步骤，就是我们传说中的这个回表了**，它是存在于主键索引数据页里，它需要通过我们索引关联过去，然后再把数据返回查询这个数据库的引擎
...
解决方案： 来压缩我们入参的这个报文， 然后再在接收端解压缩，这样可以顺利的通过网关，这仅仅也是属于一个个临时方案。 也就是说如果后续请求入参变多，同样可能超过10兆，所以压缩的方式只能说暂时解决问题，他解决不了长期的数据增长的问题

**比较长久的方案**

采用分片处理将入参的这个json分成多份进行传输, 然后在后端统一来做拼接.  就是把大的入参分成多次来调, 这种属于间接的处理方式

**cons**

远远没有压缩啊改网关来的那么快，对于发送端，接收端也会有一定的影响。 基于现有的API，进行分页的改造才能够实现。而且多次的分页调用在性能上面肯定会有影响的
...
* 服务器负载上升，没有足够的资源，访问线程处于阻塞的状态，自然访问就会变慢
* 突然的这个流量增加或者其他的一些什么应用程序， 占用了此较多的这个服务资源，**很有可能就导致了服务器的资源占用过高**， 导致负载过大
...
可能会被无效的这个请求占用殆尽，正常请求进不来，也会访问变慢

还有就是硬件本身的问题

* 比如说像这个网络问题，突发的网络中断，路由器故障啊
* 还就是像CPU过热
* 磁盘故障
* 内存泄漏
